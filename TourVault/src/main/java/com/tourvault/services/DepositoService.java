...
public class DepositoService {
    ...
    public void salvaDeposito(Deposito d) {
        try {
            ...
            if (lockerAssociato.isPresent()) {
                ...
                if (locker.getStato().equals(StatoLocker.LIBERO)) {
                	...
                	throw new MyAPIException(HttpStatus.BAD_REQUEST, "Il locker selezionato è occupato");
                }
                
            } else {
                throw new MyAPIException(HttpStatus.BAD_REQUEST, "nessun locker associato");
            }
            dr.save(d);
        } catch (Exception e) {
        	throw new MyAPIException(HttpStatus.NOT_ACCEPTABLE, "Prenotazione non riuscita");
        }
    }
